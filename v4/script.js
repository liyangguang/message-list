...
var messagesApp = {
  ...
  isScrolling: false,
  // scrollTimeout: setTimeout(this.infiniteScroll, 100),
  removeTimeouts: [],
  cardDOM: {},
  debug: function() {
  ...
  init: function() {
    ...
    // event listeners
    $('.js-main-container').scroll(this.infiniteScroll); // performance improvement: use handler 'scrollCheck' instead of 'infiniteScroll'.
  ...
  scrollCheck: function() {
    if (messagesApp.scrollTimeout) {
      clearTimeout(messagesApp.scrollTimeout);
      messagesApp.scrollTimeout = null;
    }
    messagesApp.scrollTimeout = setTimeout(messagesApp.infiniteScroll, 100);
  },
  ...
  touchStart: function(event) {
    if (!messagesApp.isScrolling) {
      ...
      messagesApp.cardDOM.cardHeight = $(messagesApp.cardDOM).height() + 40; // 40 = 16(padding) * 2 + 8(margin)
    }
  },
  ...
  dismissCard: function() {
    ...
    .delay(100).queue(function() { // move following card up
      $(this).nextAll().addClass('transition').css({
        '-webkit-transform': 'translate3d(0, -' + messagesApp.cardDOM.cardHeight + 'px, 0)',
        'transform': 'translate3d(0, -' + messagesApp.cardDOM.cardHeight + 'px, 0)'
      });
      messagesApp.showUndoSnackbar();
      $(this).dequeue();
    })
    .delay(200).queue(function() { // clean transformation
  ...
  showUndoSnackbar: function() {
    var notification = document.querySelector('.mdl-js-snackbar');
    var data = {
      message: 'Message Dismissed',
      actionHandler: messagesApp.undoDismiss,
      actionText: 'Undo',
      timeout: 1000
    };
    notification.MaterialSnackbar.showSnackbar(data);
  },
  undoDismiss: function(event) {
    messagesApp.cardDOM
    .queue(function() {
      $(this).nextAll().removeClass('transition').css({ // keep following cards' position
        '-webkit-transform': 'translate3d(0, -' + messagesApp.cardDOM.cardHeight + 'px, 0)',
        'transform': 'translate3d(0, -' + messagesApp.cardDOM.cardHeight + 'px, 0)'
      });
      $(this).css({ // move the Undo'ed card back
        'display': 'block',
        '-webkit-transform': 'translate3d(0, 0, 0)',
        'transform': 'translate3d(0, 0, 0)'
      }).dequeue();
    })
    .delay(0).queue(function() { // following cards move down
      $(this).nextAll().addClass('transition').css({
        '-webkit-transform': 'translate3d(0, 0, 0)',
        'transform': 'translate3d(0, 0, 0)'
      });
      $(this).css({ // show the Undo'ed one
        'opacity': '1',
      }).dequeue();
    });
  },
...
templates['msg'] = template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "<li class=\"js-message md-card mdl-shadow--3dp\"><img class=\"md-profile-img left profile-pic\" src=\"http://message-list.appspot.com"
